        ...
        <Card sx={{ backgroundImage: "none", backgroundColor: theme.palette.background.alt, borderRadius: "0.55rem" }}>
            ...
            <Collapse in={isExpanded} timeout="auto" unmountOnExit sx={{ color: theme.palette.neutral[300] }}>
                <CardContent>
                    ...
                    <Typography>Yearly Units Sold: {stat.yearlyTotalSoldUnits}</Typography>
                </CardContent>
            </Collapse>
        </Card>
    );
...
export default function Products() {
    ...
    if (isLoading) {
        return <Box m="1.5rem 2.5rem">Loading...</Box>;
    }

    if (error) {
        return <Box m="1.5rem 2.5rem">An error occurred: {error.message}</Box>;
    }
    ...
    return (
        <Box m="1.5rem 2.5rem">
            ...
            {data.length ? (
                ...
                <Box mt="20px">No products available</Box>
            )}
        </Box>
    );
}
